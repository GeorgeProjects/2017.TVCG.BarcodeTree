define([
  ...
  return Mn.ItemView.extend(_.extend({
      ...
      add_collapsed_triangle: function () {
        ...
        setTimeout(function () {
          self.d3el.selectAll('.collapse-triangle').remove()
      ...
      add_single_collapsed_triangle: function (nodeId) {
        ...
        function append_triangle(node_id, subtree_height, subtree_width) {
          ...
          //  在svg上面绘制path
          var barcodeNodeColorArray = Variables.get('barcodeNodeColorArray')
          var lineGraph = self.d3el.select('#barcode-container')
            .append("path")
            .attr('class', 'collapse-triangle')
            .attr("d", lineFunction(lineData))
            .attr("stroke", "black")
            .attr("stroke-width", 2)
            .attr('transform', 'translate(' + triangleX + ',' + triangleY + ')')
        }
      ...
      node_selection_click_handler: function (nodeData) {
        var self = this
        var barcodeCollection = window.Datacenter.barcodeCollection
        var nodeIndex = self.get_node_index(nodeData.id)
        var siblingNodes = self.findHighlightSiblingNodes(nodeData, nodeIndex)
      ...
      subtree_selection_click_handler: function (nodeData) {
        ...
        var barcodeCollection = window.Datacenter.barcodeCollection
        var nodeIndex = self.get_node_index(nodeData.id)
        var childrenNodes = self.findHighlightChildrenNodes(nodeData, nodeIndex)
        var siblingNodes = self.findHighlightSiblingNodes(nodeData, nodeIndex)
      ...
      add_node_dbclick_click_handler: function () {
        ...
        cc.on('click', function (el) {
          ...
          addCurrentEditIcon(el.srcElement)
        ...
        cc.on('dblclick', function (el) {
          ...
          self.trigger_mouseout_event()
          // var locX = +d3.select(el.srcElement).attr('x')
          // var barWidth = +d3.select(el.srcElement).attr('width')
          // var locY = +d3.select(el.srcElement).attr('y')
          // var barHeight = +d3.select(el.srcElement).attr('height')
          // var toolbarHeight = +$('#toolbar-view').height()
          // var histogramViewHeight = +$('#histogram-view').height()
          // var supertreeViewHeight = +$('#supertree-view').height()
          // var barcodePaddingLeft = self.barcodePaddingLeft
          // var topViewHeight = toolbarHeight + histogramViewHeight
          // var barcodeTreeYLocation = +treeDataModel.get('barcodeTreeYLocation')
          // $('#barcode-node-config').css({visibility: 'visible'})
          // $('#barcode-node-config').css({
          //   left: (locX + barWidth + barcodePaddingLeft),
          //   top: (barcodeTreeYLocation + topViewHeight + locY + supertreeViewHeight + barHeight)
          // })
        });
        ...
        function addCurrentEditIcon(src_element) {
       ...
       *  显示options按钮的方法
       */
      add_singletree_node_options_button: function (nodeData, thisNodeObj) {
        var self = this
        var DURATION = 500
        var barcodeCollection = window.Datacenter.barcodeCollection
        var treeDataModel = self.model
        var nodeObjId = nodeData.id
        var nodeObjDepth = nodeData.depth
        self.clickedObject = thisNodeObj
        var thisX = +d3.select(thisNodeObj).attr('x')
        var thisY = +d3.select(thisNodeObj).attr('y') - 1
        var thisWidth = +d3.select(thisNodeObj).attr('width')
        var iconSide = Variables.get('superTreeHeight') * 0.3
        ...
        var iconPadding = 2
        var barcodeConfigFontSize = 12
        var nodeSelectIconName = 'select-node'
        var treeSelectIconName = 'select-tree'
        var collapseIconName = 'click-collapse'
        var alignTreeIconName = 'align-tree'
        var showSummaryIconName = 'show-summary'
        var nodeNumComparisonIconName = 'node-num-comparison'
        var barcodeTreeYLocation = treeDataModel.get('barcodeTreeYLocation')
        var barcodeHeight = +treeDataModel.get('barcodeNodeHeight')
        var iconInitY = 0
        if (barcodeTreeYLocation === 0) {
          iconInitY = iconSide
        }
        var align_state = barcodeCollection.get_aligned_state(nodeObjId, nodeObjDepth)
        var iconArray = []
        if (align_state) {
          iconArray = [
            {'iconName': nodeSelectIconName, 'iconCode': '\uf111'},
            {'iconName': treeSelectIconName, 'iconCode': '\uf0e8'},
            {'iconName': alignTreeIconName, 'iconCode': '\uf06e'},
            {'iconName': showSummaryIconName, 'iconCode': '\uf038'},
            {'iconName': nodeNumComparisonIconName, 'iconCode': '\uf141'}
          ]
        } else {
          iconArray = [
            {'iconName': nodeSelectIconName, 'iconCode': '\uf111'},
            {'iconName': treeSelectIconName, 'iconCode': '\uf0e8'},
            {'iconName': collapseIconName, 'iconCode': '\uf052'},
            {'iconName': alignTreeIconName, 'iconCode': '\uf06e'}]
        }
        var iconLeftX = thisX + thisWidth / 2 - iconSide * iconArray.length / 2
        ...
        //  子树或者节点的选择状态
        // 返回的子树或者节点的选择状态主要包括 NOT_SELECTED NODE SUBTREE
        var selectionState = barcodeCollection.get_selection_state(nodeObjId, nodeObjDepth)
        //  子树伸展状态 返回true或者false
        var collapsedState = barcodeCollection.get_collapse_state(nodeObjId, nodeObjDepth)
        //  子树对齐状态 返回true或者false
        var alignedState = barcodeCollection.get_aligned_state(nodeObjId, nodeObjDepth)
        //  当前是否展示summary的柱状视图
        var summaryState = barcodeCollection.get_summary_state(nodeObjId)
        //  当前子树是否比较子树数目的状态
        var numComparisonState = barcodeCollection.get_node_num_comparison_state(nodeObjId)
        //  更新barcode的背景矩形的位置
        // var barcodeIconBg = self.d3el.select('#barcode-container')
        //   .selectAll('.barcode-icon-bg')
        //   .data(iconArray)
        var barcodeIconBg = d3.select('.barcode-tree-single-g').selectAll('.barcode-icon-bg').data(iconArray)
        barcodeIconBg.enter()
          .append('rect')
          .attr('cursor', 'pointer')
          .attr('id', function (d, i) {
        ...
        barcodeIconBg.transition()
          .duration(DURATION)
          .attr('x', function (d, i) {
            return basedX + iconLeftX + (iconSide + 1) * i
          })
          .attr('y', basedY + iconInitY - iconSide)
          .attr('height', iconSide - 1)
          .attr('width', iconSide)
          .style('opacity', 1)
        barcodeIconBg.exit().remove()
        //  更新barcode的图标的位置
        // var barcodeIcon = self.d3el.select('#barcode-container')
        //   .selectAll('.barcode-g-icon')
        //   .data(iconArray)
        var barcodeIcon = d3.select('.barcode-tree-single-g').selectAll('.barcode-g-icon').data(iconArray)
        barcodeIcon.enter()
          .append('text')
          .attr('text-anchor', 'middle')
          .attr('dominant-baseline', 'middle')
          ...
          .attr('width', iconSide)
          .style('fill', function (d, i) {
            return icon_fill_handler(d, this, selectionState, collapsedState, alignedState, summaryState, numComparisonState)
          })
          .text(function (d, i) {
            return d.iconCode
          })
          ...
          .on('click', function (d, i) {
            self.single_tree_option_controller(self, nodeData, d, this)
          })
          .style('opacity', 0)
        barcodeIcon.transition()
          .duration(DURATION)
          .attr('x', function (d, i) {
            var iconX = basedX + iconLeftX + (iconSide + 1) * i + iconSide / 2
            return iconX
          })
          .attr('y', basedY + iconInitY - iconSide / 2)
          .attr('height', iconSide - 1)
          .attr('width', iconSide)
          .style('opacity', 1)
        barcodeIcon.exit().remove()
        function icon_fill_handler(d, iconObj, selection_state, collapsed_state, align_state, summary_state, num_comparison_state) {
          if (d.iconName === nodeSelectIconName) {
            if (selection_state === Config.get('CONSTANT')['NODE']) {
              selection_highlight(iconObj)
            } else if (selection_state === Config.get('CONSTANT')['NOT_SELECTED']) {
              selection_unhighlight(iconObj)
            }
          } else if (d.iconName === treeSelectIconName) {
            if (selection_state === Config.get('CONSTANT')['SUBTREE']) {
              selection_highlight(iconObj)
            } else if (selection_state === Config.get('CONSTANT')['NOT_SELECTED']) {
              selection_unhighlight(iconObj)
            }
          } else if (d.iconName === collapseIconName) {
            if (collapsed_state) {
              selection_highlight(iconObj)
            } else {
              selection_unhighlight(iconObj)
            }
          } else if (d.iconName === alignTreeIconName) {
            if (align_state) {
              selection_highlight(iconObj)
            } else {
              selection_unhighlight(iconObj)
            }
          } else if (d.iconName === showSummaryIconName) {
            if (summary_state) {
              selection_highlight(iconObj)
            } else {
              selection_unhighlight(iconObj)
            }
          } else if (d.iconName === nodeNumComparisonIconName) {
            if (num_comparison_state) {
              selection_highlight(iconObj)
            } else {
              selection_unhighlight(iconObj)
            }
        ...
        //  icon设置为被选中的状态。
        function selection_highlight(iconObj) {
          d3.select(iconObj).classed('icon-selection-highlight', true)
        }

        //  取消对于icon的选择状态
        function selection_unhighlight(iconObj) {
          d3.select(iconObj).classed('icon-selection-highlight', false)
        }
      },
      /**
       *  单个树上的option按钮的控制函数
       */
      single_tree_option_controller: function (global_obj, nodeData, d, iconObj) {
        var self = global_obj
        var barcodeCollection = window.Datacenter.barcodeCollection
        var treeDataModel = self.model
        var nodeObjId = nodeData.id
        ...
        var selection_state = barcodeCollection.get_selection_state(nodeObjId, nodeObjDepth)
        //  子树伸展状态 返回true或者false
        var collapsed_state = barcodeCollection.get_collapse_state(nodeObjId, nodeObjDepth)
        //  子树对齐状态 返回true或者false
        var align_state = barcodeCollection.get_aligned_state(nodeObjId, nodeObjDepth)
        //  当前是否展示summary的柱状视图
        var summary_state = barcodeCollection.get_summary_state(nodeObjId)
        //  当前子树是否比较子树数目的状态
        var numComparisonState = barcodeCollection.get_node_num_comparison_state(nodeObjId)
        var nodeSelectIconName = 'select-node'
        var treeSelectIconName = 'select-tree'
        ...
        var showSummaryIconName = 'show-summary'
        var nodeNumComparisonIconName = 'node-num-comparison'
        var nodeSelectIconSrcElement = self.d3el.select('.barcode-g-icon#' + nodeSelectIconName)[0][0]
        var treeSelectIconSrcElement = self.d3el.select('.barcode-g-icon#' + treeSelectIconName)[0][0]
        if (d.iconName === nodeSelectIconName) {
          if (selection_state === Config.get('CONSTANT')['NODE']) {
            //  当前处在node selection的状态
            self.unselection_click_handler(nodeData)
            ...
            //  当前处在node unselection的状态
            self.node_selection_click_handler(nodeData)
            // selection_unhighlight(self.d3el.select())
            //  当前的nodeObj是node, 所以需要取消tree的highlight
            selection_highlight(iconObj)
            selection_unhighlight(treeSelectIconSrcElement)
          }
        } else if (d.iconName === treeSelectIconName) {
          if (selection_state === Config.get('CONSTANT')['SUBTREE']) {
            //  当前处在subtree selection的状态
            ...
            //  当前处在subtree unselection的状态
            self.subtree_selection_click_handler(nodeData)
            //  当前的nodeObj是tree, 所以需要取消node的highlight
            selection_highlight(iconObj)
            selection_unhighlight(nodeSelectIconSrcElement)
          }
        } else if (d.iconName === collapseIconName) {
          if (collapsed_state) {
            //  当前处在collapsed的状态
            self.subtree_uncollapse_handler(nodeData)
            selection_unhighlight(iconObj)
          } else {
            //  当前处在uncollapse的状态
            self.subtree_collapse_handler(nodeData)
            selection_highlight(iconObj)
          }
        } else if (d.iconName === alignTreeIconName) {
          if (align_state) {
            ...
            self.subtree_unalign_handler(nodeData, finishRemoveAlignDeferObj)
            selection_unhighlight(iconObj)
          } else {
            //  当前处在unalign的状态, 用户点击subtree是对齐当前的subtree
            //  用户点击当前的subtree的时候, 有可能当前点击节点的父亲节点是处在align状态的, 所以需要用户首先去除父亲节点的aling状态, 然后再次对齐当前的subtree的节点
            //  首先获取当前节点的处于对齐状态的最上层父亲节点
            var alignedUpperParent = get_aligned_uppest_parent(nodeData)
            ...
            if (alignedUpperParent == null) {
              ...
              self.subtree_align_handler(nodeData, finishAlignDeferObj)
              selection_highlight(iconObj)
            } else {
              ...
              self.subtree_unalign_handler(alignedUpperParent, finishRemoveAlignDeferObj)
            }
          }
        } else if (d.iconName === showSummaryIconName) {
          if (summary_state) {
            //  当前的summary是展示出来的, 用户可以删除summary的柱状图
            var changeSummaryState = !summary_state
            self.trigger_remove_summary_state(nodeObjId)
            window.Datacenter.barcodeCollection.set_summary_state(nodeObjId, changeSummaryState)
            selection_unhighlight(iconObj)
          } else {
            //  当前的summary没有展示出来, 用户点击显示summary的柱状图
            var changeSummaryState = !summary_state
          ...
          if (numComparisonState) {
            //  当前处于节点比较的状态, 需要切换成当前没有比较的状态
            var changeNumComparisonState = !numComparisonState
            window.Datacenter.barcodeCollection.set_node_num_comparison_state(nodeObjId, changeNumComparisonState)
            selection_unhighlight(iconObj)
            //  重新更新barcode的节点数据, 并且更新视图
            window.Datacenter.barcodeCollection.compute_aligned_subtree_range()
            window.Datacenter.barcodeCollection.update_all_barcode_view()
            self.trigger_super_view_update()
          } else {
        ...
        //  获取最上层对齐的父亲节点
        function get_aligned_uppest_parent(node_data) {
          var nodeIndex = self.get_node_index(node_data.id)
          var fatherCurrentNodes = self.findHighlightFatherAndCurrentNodes(node_data, nodeIndex)
          for (var fI = (fatherCurrentNodes.length - 1); fI >= 1; fI--) {
            if (barcodeCollection.get_aligned_state(fatherCurrentNodes[fI].id, fatherCurrentNodes[fI].depth)) {
