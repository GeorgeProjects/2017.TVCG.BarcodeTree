define([
...
], function (require, Mn, _, Backbone, d3, d3BarChart, Config, Variables, SVGBase) {
		'use strict'
		...
		return Mn.ItemView.extend(_.extend({
				...
				onShow: function () {
						$(self.el).mousemove(function () {
								console.log('mouseover histogram-main-svg')
						})
								.mouseout(function () {
										console.log('mouseout histogram-main-svg')
								})
				},
				init_events: function () {
						...
						self.listenTo(Variables, 'change:selectedBarcodeTreeArray change:filterBarcodeTreeIdArray', self.update_barcodetree_selection)
				},
				...
				//  鼠标离开histogram的bar上, 可以取消对于barcode的高亮
				trigger_remove_barcode_tree_event: function (barcodeTreeIdArray) {
						Backbone.Events.trigger(Config.get('EVENTS')['REMOVE_BARCODE_NODE'], {
								'barcodeTreeIdArray': barcodeTreeIdArray
						})
				},
				/**
					* 初始化视图中的svg,包括大小,位置
				...
				init_view: function () {
						...
						var margin = {
								top: window.rem_px * 1.5,
								right: window.rem_px * 2,
								bottom: window.rem_px * 2,
								left: window.rem_px * 3.5
						}
				...
				draw_histogram: function (histogramWidth, histogramHeight, margin) {
						...
						// console.log('fileInfoData length', fileInfoData.length)	
						// var sumRecords = 0					
						// for (let i = 0;i < fileInfoData.length;i++) {
						// 	sumRecords = sumRecords + fileInfoData[i].num
						// }
						// console.log('sumRecords', sumRecords)
						fileInfoData = self.sort_file_info(fileInfoData)
				...
				//	对于fileInfo数组进行排序
				sort_file_info: function (fileInfoData) {
						var self = this
						var fileInfoData = fileInfoData.sort(function (fileInfo_a, fileInfo_b) {
								// 第一个时间
								var dateAName = fileInfo_a.name
								var dateAArray = dateAName.split('-')
								var dateA = dateAArray[1]
								var dateAStr = dateA.replaceAll('_', '-')
								var dateA = new Date(dateAStr)
								//	第二个时间
								var dateBName = fileInfo_b.name
								var dateBArray = dateBName.split('-')
								var dateB = dateBArray[1]
								var dateBStr = dateB.replaceAll('_', '-')
				...
				unSelectBarItem: function (barIdArray) {
						...
						self.trigger_remove_barcode_tree_event(barIdArray)
				...
				requestData: function (selectedItemsArray) {
						...
						barcodeCollection.update_barcode_attr()
				...
				brushSelectionItems: function () {
						...
						Variables.set('selectedItemColorObj', selectedItemColorObj)
						var selectItemNameArray = Variables.get('selectItemNameArray')
						//	判断当前选择的barcodeTree是否已经都被选择
						var itemAllExisted = true
						for (var sI = 0; sI < selectedItemsArray.length; sI++) {
								var selectingItem = selectedItemsArray[sI]
								if (selectItemNameArray.indexOf(selectingItem) === -1) {
										itemAllExisted = false
										break
								}
						}
						//	如果不是所有的元素都被选择, 存在一些没有被选择的元素, 那么需要重新request数据, 否则不重新请求数据
						if (!itemAllExisted) {
				...
				clear_all_items: function () {
						...
						var selectItemNameArray = Variables.get('selectItemNameArray')
						self.trigger_remove_barcode_tree_event(selectItemNameArray)
						//  取消当前的高亮
				...
				//	更新对于barcodeTree的选择
				update_barcodetree_selection: function () {
						var self = this
						...
						for (var sI = 0; sI < selectedBarcodeTreeArray.length; sI++) {
								...
								d3.select(self.el).selectAll('#' + barcodeTreeId)
										.classed('brush-selection', true)
						}
						//	高亮从distribution中选择的barcodeTree的label
