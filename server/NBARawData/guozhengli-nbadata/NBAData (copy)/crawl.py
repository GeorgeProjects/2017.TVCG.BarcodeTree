...
def saveFile(file_name, json_obj):
	fileHandler = open(file_name + '.json', 'w')
	pickle.dump(json_obj, fileHandler)
	...
	while(True):
		...
		try:
			...
			try:
				...
				summaryPagePath = browser.find_element_by_xpath(summaryPath).get_attribute('href')
				...
				playersPagePath = browser.find_element_by_xpath(playersPath).get_attribute('href')
