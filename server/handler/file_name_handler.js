...
function read_directory_file(dataSetName, dirname, readFileDirectory, fileReadEnd, dataProcess, onError) {
		fs.readdir(dirname, function (err, filenames) {
				...
				var fileInfoArray = []
				var initDepth = 0
				//  需要使用maxDepthObj记录树的深度, 因为传递对象改变的是同一个值
				...
				dataProcess(idIndexObj) // compactDataSetObj, compactLinearObj
				fileReadEnd(fileInfoObj, dataSetObj, linearObj)
		});
}
/**
	*  删除树对象的叶子节点, 因为叶子节点通常对齐没有作用
	*/
function remove_leaf_nodes(fileObject) {
		if (typeof (fileObject.children) !== 'undefined') {
				var childrenNodes = fileObject.children
				var categoryNameUnExisted = true
				for (var cI = 0; cI < childrenNodes.length; cI++) {
						if (typeof (childrenNodes[cI].categoryName) !== 'undefined') {
								categoryNameUnExisted = false
								break
						}
				}
				//	如果所有的child节点中的categoryName属性都不存在, 即categoryNameUnExisted为true
				if (categoryNameUnExisted) {
						delete fileObject.children
				} else {
						for (var cI = 0; cI < childrenNodes.length; cI++) {
								remove_leaf_nodes(childrenNodes[cI])
						}
				}
		...
		function inner_traverse_original_tree(originalTreeNodeObj, treeObj, initDepth) {
				...
				if (typeof (treeObj.children) !== 'undefined') {
						for (var tI = 0; tI < treeObj.children.length; tI++) {
								if (typeof (treeObj.children[tI].categoryName) !== 'undefined') {
