 ...
 */
function loadOriginalSingleData (treeObj, barcodeWidthArray, fileObjData, selectedLevels, barcodeHeight) {
  var treeNodeArray = []
  var initDepth = 0
  ...
  var treeNodeLocArray = computeOriginalNodeLocation(treeNodeArray, barcodeWidthArray, selectedLevels, barcodeHeight)
 ...
 */
function loadCompactSingleData (treeObj, barcodeWidthArray, fileObjData, selectedLevels, barcodeHeight, compactNum, maxDepth) {
  ...
  for (var compactDepth = 0; compactDepth >= 0; compactDepth--) { // compactDepth >= 1 maxDepth - 1
    ...
    var compactTreeNodeLocArray = computeCompactNodeLocation(compactTreeNodeArray, barcodeHeight, barcodeWidthArray, compactNum, selectedLevels)
 ...
 * @param alignedLevel
 */
function getSuperTreeNodeFromTreeObjArray (treeObjArray, barcodeWidthArray, barcodeHeight, selectedLevels, rootId, rootLevel, alignedLevel) {
  var superTree = null
  ...
  //  根据线性化的节点序列得到节点的位置属性
  var superTreeNodeLocArray = computeOriginalNodeLocation(superTreeNodeArray, barcodeWidthArray, selectedLevels, barcodeHeight)
  var maxNodeNumTreeNodeLocArray = computeOriginalNodeLocation(maxNodeNumTreeNodeArray, barcodeWidthArray, selectedLevels, barcodeHeight)
  maxSubtreeWidth = compute_max_subtree_width(maxNodeNumTreeNodeLocArray, barcodeWidthArray)
 ...
 */
function computeCompactNodeLocation (compactTreeNodeArray, barcodeHeight, barcodeWidthArray, compactNum, selectedLevels) {
  var xLoc = 0
  var compactCount = 0
  ...
  for (var cI = 0; cI < compactTreeNodeArray.length; cI++) {
    ...
    if (compactAttr === ABSOLUTE_COMPACT_FATHER) {
      ...
      if (selectedLevels.indexOf(depth) !== -1) {
        ...
        if (rectWidth !== 0) {
          if (compactCount === 0) {
            xLoc = xLoc + rectWidth + PER_GAP_WIDTH
          }
 ...
 */
function getSuperTreeNodes (fileNameArray, existedFileObj, selectedLevelStr, barcodeWidthArray, barcodeHeight, selectedLevels, rootId, rootLevel, alignedLevel, compactNum) {
  ...
  var originalSuperTreeObj = getSuperTreeNodeFromTreeObjArray(treeObjArray, barcodeWidthArray, barcodeHeight, selectedLevels, rootId, rootLevel, alignedLevel)
  var compactSuperTreeObj = getCompactSuperTreeNodeFromTreeObjArray(compactTreeObjArray, barcodeWidthArray, barcodeHeight, selectedLevels, rootId, rootLevel, alignedLevel, compactNum)
  return {
 ...
 */
function compute_max_subtree_width (maxNodeNumTreeNodeLocArray, barcodeWidthArray) {
  ...
  for (var mI = 0; mI < maxNodeNumTreeNodeLocArray.length; mI++) {
    ...
    if (barcodeWidthArray[ depth ] !== 0) {
      maxSubtreeWidth = maxSubtreeWidth + barcodeWidthArray[ depth ] + perGapWidth
    } else {
 ...
 * @param tree: 对象形式的树
 */
function sortCompactChildren (tree) {
  innerSortCompactChildren(tree)
  function innerSortCompactChildren (tree) {
    if (typeof (tree.children) !== 'undefined') {
      ...
      for (var i = 0; i < tree.children.length; i++) {
        innerSortCompactChildren(tree.children[ i ])
      }
    }
  }
      ...
      if (!childrenArray[ cI ].isPicked) {
        ...
        for (var iI = (cI + 1); iI < childrenArray.length; iI++) {
          if (!childrenArray[ iI ].isPicked) {
            if (treeDistance(childrenArray[ cI ], childrenArray[ iI ]) === 0) {
 ...
 *
 */
function compactTreeLinearization (treeObj, initDepth) {
  var treeNodeArray = []
 ...
 */
function computeOriginalNodeLocation (treeNodeArray, widthArray, selectedLevels, barcodeHeight) {
  var xLoc = 0
  var perGapWidth = 2
  var treeNodeLocArray = []
...
function treeDistance (rootA, rootB, selectedLevels) {
  ...
  update = function (nodeA, nodeB) { return 0 }
